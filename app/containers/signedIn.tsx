...
export default class SignedIn extends Component<Props> {
	constructor(props){
		...
		console.log("is dark mode", systemPreferences)
   ...
   "selector": {
         ...
         },
         {
            "_id": {
               "$lt": "bookman-1581446666866￰"
				...
				//__DEV__ && console.log("START REPLICACTION!",id);
            	let optChapters = {
                 live: true,
                 retry: true,
                 filter: '_selector',
	...
	askSave = () => {
		...
		const dialogMBox = dialog.showMessageBox(null, options);
		dialogMBox.then(res => {
	...
	handleChange = async(text, key) => {
	 ...
	 _handleKeyDownChapter = (e) => {
	  ...
	  openBookWysiwyg = async(key) => {
	  	
	  	let c = await this.$openChapters(key);
	  	this.setState({openedBookWysiwyg:true, currentWork: key, chapters: c})
	  }
						...
						<div className={'toolsTitleBook'}>
							...
							<nav className="breadcrumbs">
									  <ul>
									  	...
									  	<li onClick={() => this.openBookWysiwyg(key.book)}><a href="#" className={'firstBread'}>{key.book.title.slice(0,30)}</a></li>
											<li>
									...
									<Popup
											onClose={() => this.setState({openedBookWysiwyg: false})}
											open={this.state.openedBookWysiwyg}

										    modal
										    closeOnDocumentClick
										  >

										     <div><div class="modalClose" onClick={() => this.setState({openedBookWysiwyg: false})}><Icon
																		  name='close'
...

												{
								  					...
								  					this._renderSpecificWork(key.book)
								  				}
								  				</div>
	...
	_renderNewTab = (key, index) => {
	    ...
	    //console.log("chapter key!!", key)
	    	...
	    	return (
				<div className={[]} key={{_id: 'welcome', title: 'New Tab', type: 'welcome', changed: false}} changed={false} id={'welcome'} title={'New Tab'} closeChapter={() => this.closeFile()}>
   ...
   _renderSpecificWork = (key) => {
		...
		//console.log("render specific work1", key)
			...
			<div className={['sidebarWorkList', 'sidebarWorkListChapters'].join(" ")} style={key._deleted && {display: 'none'}}>
				...
				<div className={sidebarWorksStyles['sidebarWorkListRowInfo']}>
					...
					<SidebarChapters 
						...
						onMoveChapters={(chapters) => this.onMoveChapters(chapters)} 
						toggleSettingsChapters={(key) => this.toggleSettingsChapters(key)} 
						onCreateChapter={() => this.onCreateChapter()} 
						chapters={this.state.chapters} 
						openChapter={this.openChapter.bind(this)} 
						currentWork={key} rootUser={this.state.rootUser}/>
				</div>
			</div>
	...
	toggle = (toState = null) =>  {
	 ...
	 onDropEpubs = async(files) => {


	 	const reader = new FileReader();

	      reader.readAsDataURL(files[0]);
	     return reader.onload = async(event) => {
	   ...
	   /* let nDocs = (this.state.works && this.state.works.rows != null) ? this.state.works.rows : [];
	    ...
	    console.log("this state works! after", this.state.works)*/
	   return this.setState({ search: '', isImporting: false, isPopoverOpen: false })
	...
	render(){
	 ...
	 if(this.state.rootUser != null){
	 	...
	 	let opwh = openedFiles && openedFiles.all && openedFiles.all;

	 	return (
	  	<div style={{width: '100%', height: '100%'}}>
	  			<input id="dark-mode" className="dark-mode-checkbox visually-hidden" type="checkbox" checked={this.state.isDarkMode}/>
				  <div className={[signedInStyles['newtContainer'], "theme-container"].join(" ")} style={{zIndex: 2}}>

				  
				  	<div>
				  	<TabPanel isDarkMode={this.state.isDarkMode} ref={r => (this.tabPanel = r)} goToNewTab={() => this.goToNewTab()} addNewTab={() => this._addTab()} selectedTab={this.state.selectedTab} onTabChange={(index) => this._handleTabChange(index)}>
				  		
				  		{
				  			opwh && opwh.map((key, index) => {
...
class TabPanel extends PureComponent {
  ...
  _renderTabs() {
    ...
    if(typeof this.props.children[i] !== 'undefined' && this.props.children[i].props.id == 'welcome'){
    	tabs.unshift(
        <Tab
          key={`tab-${i}`}
          label={'Newt'}
    ...
    } else if(typeof this.props.children[i] !== 'undefined'){
  ...
  render() {
    ...
    return (
      <div className="tab-panel" >
        <div className={["tab-panel__header", "noselect", this.props.isDarkMode == true && "tab-panel__header_dark"].join(" ")}   style={{paddingLeft: os.platform() == 'darwin' ? 55 : 0,paddingRight: os.platform() == 'darwin' ? 0 : 55 }}>
        	<div className="tab-panel__header_overflow">
        		
        		<div className={['tab', this.props.selectedTab == null && 'tab--selected', this.props.isDarkMode == true && 'tab--dark'].join(" ")} style={{width: 30}} onClick={() => this._goToNewTab()}>
			        <div className={["tab__label", "truncate"].join(" ")}>
			        	<div className="icon__mask icon__mask_bg">
			        		<img src={svglogo} style={{height: 20}} />
			             </div>
			        </div>
			      </div>
          		...
          		{/*<div className={'tab'} style={{width: 30}} onClick={() => this._addNewTab()}>
			        <div className={["tab__label", "truncate"].join(" ")}>
			        	<Icon
							  ...
							  size={15}
							  style={{marginRight: 3, marginLeft: 0, marginTop: 4}}
							/>
			        </div>
			      </div>*/}
        ...
        <div className="tab-panel__content">
          { this.props.children && this._renderTabContent()
          }
        </div>
      </div>
   );
...
class Tab extends PureComponent {
  ...
  render() {
    const tabClassName = this.props.isSelected ? 'tab tab--selected '+(this.props.isDarkMode == true ? 'tab--dark' : '') : 'tab '+(this.props.isDarkMode == true ? 'tab--dark' : '');
      ...
      <div className={tabClassName} onClick={() => this._handleClick()}>
        <div className={["tab__label", "truncate"].join(" ")}>
        {
        	...
        	<Icon
				name='ios-close'
				font='Ionicons'
				color={this.props.isUpdated == true ? '#c74e4d' : 'rgba(153, 153, 153, 0.61)'}
				size={18}
				style={{marginRight: 8}}
			/>
			</span>
